...
# For Binary
for i in range (0, len(output)):
    for x in range(10, win_step, 20):
        prob_window = output[i][x-10:x+10]
        prob_window = prob_window[:, 0]
        win_peaks, properties = find_peaks(prob_window, height=0.25)
        win_relu_output = [0] * 20

        for y in win_peaks:
            win_relu_output[y] = 1

        relu_flat_output = relu_flat_output + win_relu_output
